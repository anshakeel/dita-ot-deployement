  ...
  <xsl:template match="*" mode="step-elements-with-stepsection">
    ...
    <xsl:for-each select="*">
      <xsl:choose>
        ...
        <xsl:when test="not(contains(@class,' task/stepsection ') or contains(@class,' task/step ')) and preceding-sibling::*[1][not(contains(@class,' task/stepsection ') or contains(@class,' task/step '))]">
          <!-- Do nothing, was pulled in through recursion -->
        </xsl:when>
